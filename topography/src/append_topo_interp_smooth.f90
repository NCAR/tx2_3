...
!    Output fields are appended to input dataset
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

program append_interp_smooth
  ...
  print *,'Opening ',trim(lmask_file),' for input'
  ...
  !---- Open file to append new fields to the topo file
  print *,'Opening ',trim(lmask_file),' for output'
  ierr = nf_open(lmask_file,NF_WRITE,ncid)
  if ( ierr /= 0 ) then
     print *,'ierr=',ierr
     print *,'could not open ',lmask_file
     stop
  ...
  !---- Put in redefine mode
  ierr = nf_redef(ncid)
  if ( ierr /= 0 ) then
     print *,'ierr=',ierr
     print *,'could not enter redefine model '
     ierr = nf_close(ncid)
     stop
  endif

  !---- Get the dimensions again
  ierr = nf_inq_dimid(ncid,'lonh',dimid_lonh)
  if ( ierr /= 0 ) then
     print *,'could not find dimension lonh. ierr= ',ierr
     stop
  endif

  ierr = nf_inq_dimid(ncid,'lath',dimid_lath)
  if ( ierr /= 0 ) then
     print *,'could not find dimension lath. ierr= ',ierr
     stop
  endif

  !---- Define new field variables
  ndim = 2
  dims = (/dimid_lonh,dimid_lath/)

  ierr = nf_def_var(ncid,trim(vname_depth),NF_REAL,ndim,dims,vid_depth)
  attr_name = 'longname'
  attr_valc = 'interpolated grid cell depth'
  ...
  ierr = nf_put_att_real(ncid,vid_depth,attr_name,NF_FLOAT,1,smooth_scl)
  ...
  ierr = nf_put_att_real(ncid,vid_depth,attr_name,NF_FLOAT,1,hmin)
  ...
  stop
end program append_interp_smooth
